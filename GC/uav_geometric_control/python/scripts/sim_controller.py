...
if __name__ == "__main__":
    ...
    command_val = np.array(command_val)
    t = np.array(t)
    # def animate(i):
    #fig = plt.figure()
    ...
    dataframe = pd.DataFrame({'t':t,'xd_x':xd[:,0],'xd_y':xd[:,1], 'xd_z':xd[:,2],
                              'xddot_x':xd_dot[:,0],'xddot_y':xd_dot[:,1], 'xddot_z':xd_dot[:,2],
                              'sim_x':sim[:,0],'sim_y':sim[:,1], 'sim_z':sim[:,2],
                              'simdot_x':sim[:,3],'simdot_y':sim[:,4], 'simdot_z':sim[:,5],
                              'f': command_val[:,0], 'M1': command_val[:,1],
                              'M2': command_val[:,2], 'M3': command_val[:,3]})
