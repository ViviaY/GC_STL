...
def test():
    ...
    x0 = np.array([2, 2, 2, 0, 0, 0, 0, 0, 0])*0.1 # x,y,z, vx, vy, vz, ax,ay, az
    # x0 = np.array([[-0.5], [1.5], [-2], [3], [-2], [3]])
    goal = np.array([49, 47.5, 47.5, 0, 0, 0, 0, 0, 0])*0.1
    # goal = np.array([[-48], [50], [-46.5], [48.5], [-46.5], [48.5]])
    T = 20. 
    vmax = 2.0
    amax = 2.0
    num_segs = 40
    ...
    if PWL[0] is not None: 
        ...
        dataframe.to_csv("Ztunnel.csv",index=True,sep=',')
